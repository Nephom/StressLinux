...
func RunMemoryStressTest(wg *sync.WaitGroup, stop chan struct{}, errorChan chan string, config MemoryConfig, perfStats *config.PerformanceStats) {
	...
	if config.Debug {
		...
		utils.LogMessage(fmt.Sprintf("  - Sequential write: %.2f MB/s", writeSpeedMBps), config.Debug)
		utils.LogMessage(fmt.Sprintf("  - Sequential read:  %.2f MB/s", readSpeedMBps), config.Debug)
		utils.LogMessage(fmt.Sprintf("  - Random access:    %.2f MB/s", randomSpeedMBps), config.Debug)
	}
