...
func getDetailedSystemInfo() DetailedSystemInfo {
	...
	if err == nil && len(cpuInfo) > 0 {
		...
		for i, cpu := range cpuInfo {
			cpuDetails = append(cpuDetails, fmt.Sprintf(
				"[CPU] %d: Model: %s, Cores: %d, Frequency: %.2f MHz, Cache: %d KB",
				i, cpu.ModelName, cpu.Cores, cpu.Mhz, cpu.CacheSize))
		}
		details.CPUDetails = strings.Join(cpuDetails, "\n")
	} else {
	...
	// Detailed PCI Devices information
	if out, err := exec.Command("lspci").Output(); err == nil {
		lines := strings.Split(string(out), "\n")
		var pciDetails []string
		for _, line := range lines {
			if strings.TrimSpace(line) != "" {
				parts := strings.SplitN(line, " ", 2)
				if len(parts) == 2 {
					pciDetails = append(pciDetails, fmt.Sprintf("PCI Slot: %s, Device: %s", parts[0], parts[1]))
				}
			}
		}
		details.PCIDetails = strings.Join(pciDetails, "\n")
	} else {
		details.PCIDetails = "[PCI]: Unable to retrieve PCI device information (requires lspci)"
	}
...
func PrintSystemInfo(info SystemInfo) {
	...
	fmt.Println("\nPCI Devices:")
	fmt.Println(info.DetailedInfo.PCIDetails)
}
